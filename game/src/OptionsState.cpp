...
OptionsState::OptionsState(SDL_Renderer *r, int width, int height,
                           ResourceManager *res, SDL_Window * window, int *volume) {
    ...
    if (e->type == SDL_KEYUP) {
        switch (e->key.keysym.sym) {
            ...
            case SDLK_LEFT:
                if (editing == 1) {
                    *masterVol -= 1;
                    ...
                    currBright -= 1;
                    if(currBright < 0) currBright = 0;
                    else {
                        int w,h;
                        SDL_DestroyTexture(bright[0]);
                        SDL_DestroyTexture(bright[1]);
                        brightnessTitle = "Brightness: " + std::to_string(currBright);
                        bright[0] = resources->getFont("manaspc60", brightnessTitle);
                        SDL_Color green = {110, 233, 69, 255};
                        resources->setTextColor(green);
                        bright[1] = resources->getFont("manaspc60", brightnessTitle);
            ...
            case SDLK_RIGHT:
                if (editing == 1) {
                    *masterVol += 1;
                    if(*masterVol > MAX_VOLUME) *masterVol = MAX_VOLUME;
                    else {
                        int w,h;
                        SDL_DestroyTexture(audio[0]);
                        SDL_DestroyTexture(audio[1]);
                    ...
                    currBright += 1;
                    if(currBright > MAX_BRIGHTNESS) currBright = MAX_BRIGHTNESS;
                    else {
                        SDL_DestroyTexture(bright[0]);
                        SDL_DestroyTexture(bright[1]);
                        int w,h;
...
void OptionsState::render(int dt) {
    ...
    if (SDL_RenderCopy(renderer, navigation, NULL, &navigationRect) <
        0) {
        std::cout << "Something broke: " << SDL_GetError();
    }
