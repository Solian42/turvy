...
const char *musicTitle = "../data/sounds/abstract_tracking.xm";
...
double radToDegs(double r) { return r * radToDeg; }
...
void load() {
  ...
  SDL_Surface *image = IMG_Load("../data/images/johns-hopkins-university-logo.png");
  if (!image) {
  ...
  gFont = TTF_OpenFont("../data/fonts/CaesarDressing-Regular.ttf", 72);

  gFont2 = TTF_OpenFont("../data/fonts/CaesarDressing-Regular.ttf", 74);
...
void run() {
  ...
  SDL_Rect shadowRect = {center(width, shadow->w), center(height, shadow->h),
                         shadow->w, shadow->h};
  SDL_Rect textRect = {center(width, text->w), center(height, text->h), text->w,
                       text->h};
